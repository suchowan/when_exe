...
module When
  module CalendarTypes
    ...
    # ベース: 前漢～蜀漢(太初暦・四分暦)
    Korean_060 = PatternTableBasedLuniSolar.patch('Chinese_103', -60..263, {
      -26 => {'aB'   => 'Ab'  },                   # ベースとする中国暦日表の違い
      240 => {'Hi'   => 'hI'  }                    # 四分・景初・乾象暦いずれにも不一致
    })

    # ベース: 南朝(元嘉暦・大明暦)
    Korean0445 = PatternTableBasedLuniSolar.patch('Chinese0445', 445..589, {
    ...
    # ベース: 隋・唐～五代前期(大業暦～調元暦)
    Korean0590 = PatternTableBasedLuniSolar.patch('Chinese0523', 590..946, {
      592 => {'FgHi' => 'fgHI'},
    ...
    # ベース: 五代後期～南宋(調元暦～本天暦)
    Korean0947 = PatternTableBasedLuniSolar.patch('Chinese0956', 947..1279, {
      1022 => {'aB'  => 'Ab'  },                   # 宣明暦に一致
    ...
    # ベース: 元・明・南明(重修大明暦・授時暦・大統暦)
    Korean1280 = PatternTableBasedLuniSolar.patch('Chinese0939', 1280..1653, {
   ...
   #- 1650 => {'kL'  => 'Kl'  },
    ...
    # ベース: 清(時憲暦)
    Korean1654 = PatternTableBasedLuniSolar.patch('Chinese1645', 1654..1911, {
   #- 1664 => {'bC'  => 'Bc'  },
   #- 1674 => {'aB'  => 'Ab'  },
      1677 => {'iJ'  => 'Ij'  },
      1698 => {'jK'  => 'Jk'  },
  ...
  class TM::CalendarEra
    ...
    Korean = [self, [
      ...
      [self,
	"period:[高句麗]",
	["[東明王]1",	"@F",	"name=[東明王];-36^Korean_060"],
	["[瑠璃王]1",	"@A",	"name=[瑠璃王];-18"],
	...
	["[中川王]1",	"@A",	"name=[中川王];248", "264^Chinese0239", ""],
	...
	["[古爾王]1",	"@A",	"name=[古爾王];234", "264^Chinese0239", ""],
	...
	["[毗有王]1",	"@A",	"name=[毗有王];427", "445^Korean0445", ""],
	...
	["[威徳王]1",	"@A",	"name=[威徳王];554", "590^Korean0590", ""],
	["[恵王]1",	"@A",	  "name=[恵王];598"],
	["[法王]1",	"@A",	  "name=[法王];599"],
	["[武王]1",	"@A",	  "name=[武王];600"],
	["[義慈王]1",	"@A",	"name=[義慈王];641", "660.08="]		# 資治通鑑による
      ...
      [self,
	"period:[新羅]",
	["[赫居世]1",	"@F",	"name=[赫居世];-56^^Korean_060"],
	...
	["[味鄒王]1",	"@A",	"name=[味鄒王];262", "264^Chinese0239", ""],
	...
	["[訥祇王]1",	"@A",	"name=[訥祇王];417", "445^Korean0445", ""],
	...
	["[真平王]1",	"@A",	"name=[真平王];579", "590^Korean0590", ""],
	...
	["[敬順王]1",	"@A",	"name=[敬順王];927", "935="],
        [self,
	  "period:[(年号)]",
	  ["[建元]1",	"",	"name=[法興王];536^Korean0445"],
	  ["[開国]1",	"",	"name=[真興王];551"],
	  ["[太昌,alias:大昌]1",	"",   "568"],
	  ["[鴻済]1",	"",		      "572"],
	  ["[建福]1",	"",	"name=[真平王];584", "590^Korean0590", ""],
	  ["[仁平]1",	"",   "name=[善徳女王];634"],
	  ["[太和]1",	"@A", "name=[真徳女王];647", "649="]
        ],
        [self,
	  "period:[長安]",		# １年で滅亡したと仮定
	  ["[慶雲]1",	"@F",	"name=[金憲昌];822^^Korean0590", "822="]
        ]
      ],
      [self,
	"period:[後百済]",		# 三国遺事による
	["[甄萱]1",	"@F",	"name=[甄萱];900^Korean0590", "936.09.08="]
      ],
      [self,
	"period:[後高句麗]",
	["[弓裔]1",	"@F",	"name=[弓裔];901^Korean0590"],
	["[摩震]1",	"@A",	"name=[武泰];904"],
	["[聖冊]1",	"",		    "905"],
	...
	["[政開]1",	"",		    "914", "918.06"]
      ...
      [self,
	"period:[高麗]",
	["[天授]1",	"@F",	"name=[太祖];918.06^Korean0590"],	# 三国遺事による
	["[太祖]16",	"",		    "933"],
	["[恵宗]1",	"@A",	"name=[恵宗];944"],
	["[定宗]1",	"@A",	"name=[定宗];946", "947^Korean0947", ""],
	["[光徳]1",	"@A",	"name=[光宗];950"],
	...
	["[高宗]1",	"@A",	"name=[高宗];1214"],
	["[元宗]1",	"@A",	"name=[元宗];1260"],
	["[忠烈王]1",	"@A",	"name=[忠烈王];1275", "1280^Korean1280", ""],
	...
	["[恭譲王]1",	"@A",	"name=[恭譲王];1390", "1392.07.17"]	# コンサイスによる
      ...
      [self,
	"period:[朝鮮]",
	["[太祖]1.07.17","@F",	"name=[太祖];1392-07-17^Korean1280"],	# コンサイスによる
	["[定宗]1",	"@A",	"name=[定宗];1399"],
	["[太宗]1",	"@A",	"name=[太宗];1401"],
	["[世宗]1",	"@A",	"name=[世宗];1419"],
	["[文宗]1",	"@A",	"name=[文宗];1451"],
	...
	["[仁祖]1",	"@A",	"name=[仁祖];1623"],
	["[孝宗]1",	"@A",	"name=[孝宗];1650", "1654^Korean1654", ""],
	["[顕宗]1",	"@A",	"name=[顕宗];1660"],
	["[粛宗]1",	"@A",	"name=[粛宗];1675"],
	["[景宗]1",	"@A",	"name=[景宗];1721"],
